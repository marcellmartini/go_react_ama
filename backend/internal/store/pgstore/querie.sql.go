...
const removeReactionsFromMEssage = `-- name: RemoveReactionsFromMEssage :one
UPDATE messages
    SET reaction_count = reaction_count -1
    WHERE id = $1
    RETURNING reaction_count
...
func (q *Queries) RemoveReactionsFromMEssage(ctx context.Context, id uuid.UUID) (int64, error) {
	row := q.db.QueryRow(ctx, removeReactionsFromMEssage, id)
	var reaction_count int64
	err := row.Scan(&reaction_count)
	return reaction_count, err
}
